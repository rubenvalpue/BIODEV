...
# check that all the clusters have 10 plots
...
# number of field measurements days
...
# there were no palms in Kou

# check IDs
#unique(palms$cluster)
#unique(palms$plot)
#unique(palms$palm)
...
# check IDs
unique(l.trees.4_10$cluster)
unique(l.trees.4_10$plot)
unique(l.trees.4_10$subplot) #should be 1-4
...
# check IDs
unique(d.trees.4_10$cluster)
unique(d.trees.4_10$plot)
unique(d.trees.4_10$subplot)
...
# only 0.3% of the trees could not be identified
