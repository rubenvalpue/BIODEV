...
# crown diameters were not measured for palms
# fixed cd has to be assumed (e.g. 3 m)
p.cd <- 3 
p["ca"] <- (pi*p.cd^2)/4

p.p <- aggregate(p$ca, by=list(p$plot.id), sum)
names(p.p) <- c("plot.id","ca")

p.p <- merge(plots, p.p, by=c("plot.id"), all.x=TRUE)
p.p[is.na(p.p$ca), "ca"] <- 0

p.p["c.cr"] <- p.p$ca/(pi*p.p$radius^2)*100
p.p <- p.p[,c("plot.id","c.cr")]
summary(p.p$c.cr)


##########################
#MERGE LARGE TREES, SMALL TREES AND PALMS
##########################
...
# add palms
cc.p <- merge(cc.p, p.p, by=c("plot.id"), all.x=TRUE)
names(cc.p) <- c("plot.id","lt.c.cr","st.c.cr","p.c.cr")
cc.p[is.na(cc.p$p.c.cr), "p.c.cr"] <- 0
...
# palms in the data
cc.p["c.cr"] <- cc.p$lt.c.cr + cc.p$st.c.cr + cc.p$p.c.cr
