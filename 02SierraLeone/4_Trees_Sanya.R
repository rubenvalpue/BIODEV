...
d.wd <- mean(species$wd.mean) #mean value for the species in Sanya
...
# root:shoot ratio (Mokany et al. 2006): median root:shoot ratio for "Tropical/subtropical dry 
# forest/plantation, shoot biomass > 20 Mg/ha"
rs.mokany <- 0.275
...
###########################
#ADD WOOD DENSITY FOR TREES AND PALMS
###########################

# trees, DBH > 10 cm
trees <- merge(trees, species[,c("code","wd.mean")], by.x="sp", by.y="code", all.x=TRUE)
...
# palms
palms <- merge(palms, species[,c("code","wd.mean")], by.x="sp", by.y="code", all.x=TRUE)
...
# recommended model when tree height (H) has been measured or modelled
...
# for Sanya, Sierra Leone, E=0.402448
E <- 0.402448
...
par(mfrow=c(1,2))
plot(t$dbh, pred.c14.4, xlab="DBH (cm)", ylab="AGB (kg)", ylim=c(0,5300),
     main="Chave14, eq. 4 (DBH + H + WD)", col=as.factor(t$sp))
plot(t$dbh, pred.c14.7, xlab="DBH (cm)", ylab="AGB (kg)", ylim=c(0,5300),
...
# Frangi & Lugo (1985)
# model is based on total height
fl85.1 <- function(h) {6.4*h-10.0}

plot(palms$h, fl85.1(palms$h), main="FL85, eq.1 (Total H)", xlab="H (m)",
     ylab="AGB (kg)", col=as.factor(palms$sp), ylim=c(0,max(fl85.1(palms$h))))

graphics.off()


##########################
#COMPUTE AGB FOR EACH TREE, PALM AND PIECE OF DEAD WOOD
##########################

# Chave14 (eq.4) and heights based on "nlme2" (most accurate estimates) are used in 
# computations below. FL85 (eq.1) is used for palms as total height was measured 
# in the field. unit = kg

# trees, DBH > 10 cm
trees["agb"] <- chave14.4(trees$dbh, trees$h.nlme2, trees$wd.mean)
...
# Generic model (F=0.42) and heights based on "nlme2" (most accurate estimates) 
# are used in computations below.
# No volume estimates for palms.
# unit = m^2

# trees, DBH > 10 cm
trees["v"] <- generic.f(trees$dbh, trees$h.nlme2, 0.42)
...
d.trees.4_10["species"] <- 142 #code for unknown (also local name unknown)
d.trees.4_10["tree.type"] <- "SDT" 
d.trees.4_10["decay.class"] <- 0 #zero for standing trees

# palms
palms["height"] <- palms$h
palms["species"] <- palms$sp
palms["tree.type"] <- "living"
palms["decay.class"] <- 0 #zero for standing trees
palms["v"] <- 0 #not included for palms
palms["bgb"] <- 0 #not included
palms["bgc"] <- 0 #not included

# downed dead wood
d.wood["dbh"] <- d.wood$d #diameter in the middle
d.wood["height"] <- d.wood$length #length of the piece
d.wood["species"] <- 142 #code for unknown (also local name unknown)
...
save(TreeList, file = "TreeList_Sanya.rda")
